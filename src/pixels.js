...
import style from './pixels.module.scss';
...
class Pixels extends Component {
  ...
  render() {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 154.68 166.116">
        ...
        <g clipPath="url(#a)" transform="matrix(1.33333 0 0 -1.33333 -39.13999942 234.4946576)">
          {Object.keys(this.state.pixels).map((key) => <path key={key} className={style.path} d={key} fill={this.state.pixels[key]} />)}
        </g>
      </svg>
    );
  }
  ...
  setRandomColors() {
    this.setState({ pixels: Object.keys(this.originalPixels).reduce((newState, key) => ({...newState, [key]: this.randomColor()}), {})})
  }
